...
MultiLineComment = "/*" inner:(!"*/" i:. { return i; })* "*/" { return { type: 'comment', raw: inner.join('') }; }
...
BitExpr1
  ...
  = expr1:BitExpr2 rest:(op:BitExprOp1 expr2:BitExpr2 { return { op, expr2 }; })* {
      return rest.reduce((acc, cur) => ast.BinaryExpression(cur.op, acc, cur.expr2), expr1);
    }
...
FunctionCall
  = func:FunctionName LPAREN exprs:ExpressionList RPAREN { return ast.CallExpression(func, exprs); }

  // JSON_EXTRACT shorthand syntax (e.g. foo->'$.bar', or foo->>'$.bar')
  / ident:Identifier arrow:(ARROWW / ARROW) lit:StringLiteral {
      let rv = ast.CallExpression(ast.BuiltInFunction('JSON_EXTRACT'), [ident, lit]);
      if (arrow === '->>') {
...
DecimalNumberLiteral = digits:[0-9]+ { return ast.Literal(parseInt(digits.join(''), 10)); }

HexNumberLiteral = "0x" digits:[0-9a-fA-F]+ { return ast.Literal(parseInt(digits.join(''), 16)); }
...
SingleQuotedStringLiteral
  = "'" seq:("''" / "\\'" { return "''"; } / [^'])* "'" { return ast.Literal(unquote(seq.join(''))); }
...
Assignment = ident:Identifier second:EQ third:Expression { return [ident.name, second, third]; }
...
AlterSpec
  ...
  / ADD
    ...
    position:(AFTER ident:Identifier { return `AFTER ${ident.name}`; } / FIRST { return 'FIRST'; })? {
      return {
        type: 'ADD COLUMN',
  ...
  / CONVERT TO CHARACTER SET charset:CharsetName collate:(COLLATE collate:CollationName { return collate; })? {
      return {
        type: 'CONVERT TO',
        charset,
        collate,
...
CreateTable3
  = CREATE TABLE ifNotExists:(IF NOT EXISTS)? tblName:Identifier LIKE oldTblName:Identifier {
      return {
...
CreateDefinition
  ...
  / constraint:NamedConstraint? UNIQUE (INDEX / KEY)? indexName:Identifier? LPAREN indexColNames:IndexColNames RPAREN {
      return {
        type: 'UNIQUE INDEX',
        constraint,
        indexName: indexName?.name ?? null,
...
ColumnDefinition
  = dataType:DataType
    ...
    generated:(
      (GENERATED ALWAYS)? AS LPAREN expr:Expression RPAREN mode:(STORED / VIRTUAL)? {
...
Precision
  = LPAREN length:NumberLiteral COMMA decimals:NumberLiteral RPAREN { return { length: length.value, decimals }; }
...
Encoding
  = charset:Charset collate:Collate? { return makeEncoding(charset, collate ?? undefined); }
...
IndexColName = colName:Identifier len:Len? direction:(ASC / DESC)? { return { colName: colName.name, len, direction }; }
...
CharsetName "character set name" = _ charset:("latin1" / "utf8mb4" / "utf8") !IdentifierStart _ { return charset; }
...
CurrentTimestamp
  ...
  / (CURRENT_TIMESTAMP / LOCALTIMESTAMP / LOCALTIME) precision:(LPAREN n:NumberLiteral? RPAREN { return n; })? {
      return ast.CurrentTimestamp(precision !== null ? precision.value : null);
    }
...
QuotedIdentifier = _ "`" chars:[^`]+ "`" _ { return ast.Identifier(chars.join('')); }
...
LOCALTIMESTAMP = _ "LOCALTIMESTAMP"i !IdentifierChar _ { return 'LOCALTIMESTAMP'; }
...
CHAR_LENGTH = _ "CHAR_LENGTH"i !IdentifierChar _ { return ast.BuiltInFunction('CHAR_LENGTH'); }
...
SUBSTRING = _ "SUBSTRING"i !IdentifierChar _ { return ast.BuiltInFunction('SUBSTRING'); }

UNHEX = _ "UNHEX"i !IdentifierChar _ { return ast.BuiltInFunction('UNHEX'); }

JSON_EXTRACT = _ "JSON_EXTRACT"i !IdentifierChar _ { return ast.BuiltInFunction('JSON_EXTRACT'); }

JSON_UNQUOTE = _ "JSON_UNQUOTE"i !IdentifierChar _ { return ast.BuiltInFunction('JSON_UNQUOTE'); }
